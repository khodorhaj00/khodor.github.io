...
// (the renderer clears to transparent, so anything with alpha is model or grid) and hashes
// the whole frame so two renders can be compared.
async function frameSignature(page) {
  return page.evaluate(() => {
    ...
    return { drawn, total: data.length / 4, hash: hash >>> 0 };
  });
}
...
async function testExport(page) {
  ...
  check(event && event.payload.ok === true, `exportGlb: ok (${event ? event.payload.error || event.payload.filename : 'no event'})`);
  if (!event || !event.payload.ok) return;
  const bytes = Buffer.from(event.payload.base64, 'base64');
  checkEqual(bytes.toString('latin1', 0, 4), 'glTF', 'exportGlb: magic');
  checkEqual(bytes.readUInt32LE(4), 2, 'exportGlb: glTF version');
  checkEqual(bytes.readUInt32LE(8), bytes.length, 'exportGlb: declared length matches');
  const jsonLength = bytes.readUInt32LE(12);
  checkEqual(bytes.readUInt32LE(16), 0x4e4f534a, 'exportGlb: first chunk is JSON');
  let json = null;
  try {
    json = JSON.parse(bytes.toString('utf8', 20, 20 + jsonLength));
  } catch (error) {
    check(false, `exportGlb: JSON chunk parses (${error.message})`);
    return;
  }
  check(Array.isArray(json.meshes) && json.meshes.length > 0, `exportGlb: meshes.length = ${json.meshes ? json.meshes.length : 0}`);
  checkEqual(event.payload.filename, 'meshes.glb', 'exportGlb: filename');
...
async function main() {
  ...
  try {
    ...
    for (const fixture of FIXTURES) {
      ...
      await checkNotBlank(page, fixture.name);
